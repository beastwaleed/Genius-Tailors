...
import { SERVICE_FIELDS, STANDARD_SIZES, IMAGE_MAP, getMeasurementVisualConfig } from './Measurements';

import imgBanCollar from '../../assets/styles/ban_collar.jpg';
import imgShirtCollar from '../../assets/styles/shirt_collar.jpg';
import imgRoundNeck from '../../assets/round neck collar waistcoat.jpeg';
...
export default function Booking() {
  ...
  const applyPreset = (size) => {
    const presets = STANDARD_SIZES[inlineServiceKey]?.[size];
    if (presets) {
      setInlineMeasurements(presets);
      toast.success(`Applied ${size} standard sizing`);
      if (!inlineProfileName || inlineProfileName.includes(serviceName)) setInlineProfileName(`${serviceName} - ${size}`);
      setActiveField(null);
    }
  };
  ...
  const handleHWEstimate = (e) => {
    ...
    const newMeasurements = { ...inlineMeasurements };
    const config = SERVICE_FIELDS[inlineServiceKey];
    
    const updateLabelFields = (labelConfig) => {
      if (!labelConfig) return;
      const label = labelConfig.label;
      if (labelConfig.fields.includes('Shoulder')) newMeasurements[`${label} Shoulder`] = shoulder.toFixed(1);
      if (labelConfig.fields.includes('Sleeves')) newMeasurements[`${label} Sleeves`] = sleeves.toFixed(1);
      if (labelConfig.fields.includes('Length')) newMeasurements[`${label} Length`] = kameezL.toFixed(1);
      if (labelConfig.fields.includes('Chest')) newMeasurements[`${label} Chest`] = chest.toFixed(1);
      if (labelConfig.fields.includes('Abdomen')) newMeasurements[`${label} Abdomen`] = abdomen.toFixed(1);
      if (labelConfig.fields.includes('Hips')) newMeasurements[`${label} Hips`] = hips.toFixed(1);
      if (labelConfig.fields.includes('Collar')) newMeasurements[`${label} Collar`] = neck.toFixed(1);
      if (labelConfig.fields.includes('Bottom')) newMeasurements[`${label} Bottom`] = bottom.toFixed(1);
      if (labelConfig.fields.includes('Crotch Depth')) newMeasurements[`${label} Crotch Depth`] = crotch.toFixed(1);
    };

    updateLabelFields(config?.top);
    updateLabelFields(config?.bottom);
    updateLabelFields(config?.outer);

    if (config?.bottom?.label && config.bottom.fields.includes('Length')) newMeasurements[`${config.bottom.label} Length`] = shalwarL.toFixed(1);

    setInlineMeasurements(newMeasurements);
              ...
              <div className="modal-body interactive-body">
                ...
                <div key={activeField || 'empty'} className={`interactive-visual-side ${showMobileGuide ? 'show-mobile-guide' : ''}`}>
                  ...
                  <div className="mannequin-container">
                    <img src={IMAGE_MAP[inlineServiceKey] || IMAGE_MAP['Kameez Shalwar']} alt="Tailor Model" className="mannequin-img" />
